---
title: "psychencode_file_generation"
...
CMC_matrix = CMC_cpm[LIBD_sds > 0.1, ] %>% as.data.frame() %>% rownames_to_column( var = "gene_symbol")
```
...
hgnc_mapping = read_tsv('/external/rprshnas01/kcni/dkiss/KCNI_data_analyses/hgnc_complete_set.txt')
...
new_marker_list  = lapply(new_cell_types, function(cell_type){
  return(new_markers %>% filter(subclass == cell_type, 
                                ensembl_id %in% unique(c(GVEX_matrix$gene_symbol, LIBD_matrix$gene_symbol)),
  ) %>% pull(ensembl_id))
})
