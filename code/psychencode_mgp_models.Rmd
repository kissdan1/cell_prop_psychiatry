...
#Model cell_prop based on diagnosis in each age grouo
#Meganalysis
...
```{r}
# Create function to map age to age range based on 10% quantiles
...
snCTPs_long$age_range = snCTPs_long$ageOfDeath %>% age_to_range_quantile() %>% factor()
#Meganalysis
...
figure_1a = mega_lms %>% 
  ...
  guides(fill = FALSE) +
  scale_y_continuous(limits = c(-0.25, 0.39))
...
figure_1 + plot_annotation(tag_levels = 'A') + plot_layout(heights = c(1,1,1.5)) & theme(plot.tag = element_text(size = 25))
```
...
figure_2a = mega_lms %>% 
  ...
  theme(
    axis.text.x = element_text(angle = 45, vjust = 1, hjust= 1, size = 11), 
    axis.title.x = element_text(vjust = 8, size = 14),
    axis.title.y = element_text(size = 14),
    strip.text.x = element_text(size = 12)) +
  guides(fill = FALSE) +
...
significant_mega_age <- subset(mega_lms_by_age, padj <= 0.05 & term == "SCZ" & cell_type %in% c("Inh_SST", "Inh_PVALB", "Inh_VIP"))
significant_mega_age$asterisks <- ifelse(significant_mega_age$padj <= 0.001, "***",
                                   ifelse(significant_mega_age$padj <= 0.01, "**", "*"))
significant_mega_age$vjust = ifelse(significant_mega_age$estimate >= 0, -2.5, 4)
...
figure_2 + plot_annotation(tag_levels = 'A') + plot_layout(heights = c(1,1,0.9)) & theme(plot.tag = element_text(size = 25))

```

Figure 3: Validating trends and interactions in ssRNAseq and microscopy datasets
```{r, fig.height=8.66, fig.width=10}
...
#Figure 3b: Microscopy cell proportions binned by 20% age quantile
...
significant_micro$vjust = ifelse(significant_micro$estimate >= 0, -4, 5)

figure_3b = micro_mega_lms_by_age %>% 
...
figure_3 + plot_annotation(tag_levels = 'A') + plot_layout(heights = c(1,1)) & theme(plot.tag = element_text(size = 25))
