...
library(lemon)
=======
>>>>>>> ef77234a822c72c1fecb207d412e75933ca1a49e
=======
>>>>>>> ef77234a822c72c1fecb207d412e75933ca1a49e
=======
>>>>>>> ef77234a822c72c1fecb207d412e75933ca1a49e
...
figure_3b = snCTPs_long %>%
  ...
  theme(
    ...
    legend.title = element_blank(),
    strip.text.x = element_text(size = 16)) +
    facet_rep_grid(rows = vars(cell_type), cols = vars(dataset), drop = T, scale = "free", switch = "y", space = "free_x",repeat.tick.labels = T) +
    guides(fill = guide_legend(nrow = 1)) + 
  scale_color_manual(values=c("dodgerblue2", "firebrick2"))
...
figure_4b = microCTPs_long %>%
  filter(Subject.Group %in% c('SCHIZ', "Control"), cell_type %in% c("SST", "PV", "VIP")) %>%
  ggplot(aes(x = Age, y = rel_prop, color = Subject.Group)) +
  geom_point(alpha = 0.8, size = 1.3) + 
  geom_smooth(se = F, method = 'lm', fullrange = T) +
  scale_colour_hue(labels=c("Control", "SCZ")) +
  ylab('Relative Cell Prop.') + 
