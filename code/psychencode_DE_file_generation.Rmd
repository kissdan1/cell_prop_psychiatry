...
```{r}
#Define cohort (MtSinai or McLean)
USE_COHORT = 'MtSinai'
...
for(CELL_TYPE in CELL_TYPES) {
  ...
  UNDER_70_SCZ = pseudobulk_metadata %>% filter(unique_donor_ID %in% under_70_subjects, Celltype == CELL_TYPE, Phenotype == "SZ") %>% pull(unique_donor_ID)
  UNDER_70_CON = pseudobulk_metadata %>% filter(unique_donor_ID %in% under_70_subjects, Celltype == CELL_TYPE, Phenotype == "CON") %>% pull(unique_donor_ID)
  OVER_70_SCZ = pseudobulk_metadata %>% filter(unique_donor_ID %in% over_70_subjects, Celltype == CELL_TYPE, Phenotype == "SZ") %>% pull(unique_donor_ID)
  OVER_70_CON = pseudobulk_metadata %>% filter(unique_donor_ID %in% over_70_subjects, Celltype == CELL_TYPE, Phenotype == "CON") %>% pull(unique_donor_ID)
