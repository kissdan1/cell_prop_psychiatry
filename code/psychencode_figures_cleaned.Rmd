...
significant_mega = subset(mega_lms, padj <= 0.1 & term == "Age x Diagnosis" & cell_type %in% c("Astrocyte", "IT", "L5.6.NP", "LAMP5", "Microglia", "Pericyte", "PVALB", "SST", "VIP" ))
...
figure_2a = mega_lms %>%  filter(cell_type %in% c("Astrocyte", "IT", "L5.6.NP", "LAMP5", "Microglia", "Pericyte", "PVALB", "SST", "VIP" )) %>%
...
figure_2d = mega_lms_by_age_class %>% 
  ...
  ylab('Beta Coefficient') + 
  xlab('\n \n Age Range') + 
  scale_fill_manual(values = c("dodgerblue2", "firebrick2"), labels=c('Above 70', 'Below 70')) +
...
significant_sn_mega <- subset(sn_mega_lms, padj <= 0.1 & term == "Interaction" & cell_type %in% c("Astrocyte", "IT", "L5.6.NP", "LAMP5", "Microglia", "Pericyte", "PVALB", "SST", "VIP" ))
...
figure3_top = figure_3a + figure_3b + plot_layout(widths = c(2,3)) 
figure_3 <- figure3_top/figure_3c
figure_3 = figure_3 + plot_annotation(tag_levels = 'A') + plot_layout(heights = c(2,3)) & theme(plot.tag = element_text(size = 25))
figure_3
...
# Use ggsave to save the plot as a JPG image
ggsave(output_filename, figure_3, width = 16, height = 12, dpi = 500)
