...
# 
# acc_mega_lms_by_age = acc_estimations_long %>% 
#   group_by(age_range, cell_type) %>%
...
snCTPs_long$age_class = snCTPs_long$ageOfDeath %>% classify_ages() %>% factor(levels = c("Under 70", "Over 70"))

sn_mega_lms = snCTPs_long %>% filter(total_cell_counts >= 500) %>%
  group_by(cell_type) %>%
...
```{r}
sn_mega_lms_by_age = snCTPs_long %>% filter(total_cell_counts >= 500) %>%
...
res_model = lm(scale(rel_prop) ~ scale(RIN) + scale(PMI) + reportedGender, data = mgp_estimations_long)
mgp_estimations_long <- mgp_estimations_long %>% add_residuals(var = "resid", model = res_model)

# Figure 2c: Visualizing interaction between age and diagnosis by plotting rel_prop vs. age in each diagnosis 
figure_2c = mgp_estimations_long %>%
  filter(cell_type %in% c("SST", "PVALB", "VIP")) %>%
...
significant_sn_mega <- subset(sn_mega_lms, padj <= 0.1 & term == "Interaction" & cell_type %in% c("SST", "PVALB", "VIP"))
significant_sn_mega$asterisks <- ifelse(significant_sn_mega$padj <= 0.01, "***",
                                         ifelse(significant_sn_mega$padj <= 0.05, "**", "*"))
significant_sn_mega$vjust <- ifelse(significant_sn_mega$estimate >= 0, -3, 3)

figure_3c = sn_mega_lms %>% 
...
# Use ggsave to save the plot as a JPG image
ggsave(output_filename, figure_3, width = 13, height = 17, dpi = 200)
