...
BipSeq_matrix = read.csv("/external/rprshnas01/kcni/dkiss/cell_prop_psychiatry/data/BipSeq_count_matrix.csv")
BipSeq_matrix = cbind(GVEX_matrix[1], BipSeq_matrix)
names(BipSeq_matrix) = gsub("X", '', names(BipSeq_matrix))
names(BipSeq_matrix)[1] = "gene_symbol"
...
BipSeq_matrix$gene_symbol = gsub("\\..*", '', BipSeq_matrix$gene_symbol)
...
row.names(BipSeq_matrix) = BipSeq_matrix$gene_symbol
BipSeq_matrix = BipSeq_matrix[,-1]

#Combine into one large matrix for easier analysis
combined_matrix = cbind(GVEX_matrix, LIBD_matrix, CMC_matrix, BipSeq_matrix)
...
BipSeq_metadata = read.delim("/external/rprshnas01/external_data/psychencode/PsychENCODE/BipSeq/RNAseq/SYNAPSE_METADATA_MANIFEST.tsv") %>% filter(dataType == "geneExpression")  %>% left_join(psychencode_metadata)


#Change LIBD NAs to BipSeq, since we are using control from here ****IMPORTANT****
LIBD_metadata[which(is.na(LIBD_metadata$individualIdSource)), 'individualIdSource'] = 'BipSeq'
LIBD_metadata = LIBD_metadata %>% filter(primaryDiagnosis %in% "control", individualIdSource %in% "BipSeq")

#Change NAs in BipSeq individdualIDSource to BipSeq
BipSeq_metadata[which(is.na(BipSeq_metadata$individualIdSource)), 'individualIdSource'] = 'BipSeq' 

columns = names(GVEX_metadata) %>% intersect(names(CMC_metadata)) %>% intersect(names(BipSeq_metadata)) %>% intersect(names(LIBD_metadata))
#Combine all metadata, filter based on inclusion criteria, and add newStudy column for cohort-based MGP
combined_metadata = rbind(GVEX_metadata[, columns], CMC_metadata[, columns], BipSeq_metadata[, columns], LIBD_metadata[, columns])
#Remove '+' from ageDeath for estimation/filtering
combined_metadata$ageDeath = as.numeric(gsub("[+]", "", combined_metadata$ageDeath))
#Additional filtering 
combined_metadata = combined_metadata %>% filter(primaryDiagnosis %in% c("control", "Bipolar Disorder"), ageDeath >= 15)
...
combined_metadata = combined_metadata %>%
  mutate(newStudy = case_when(
    ...
    individualIdSource == "Pitt"                    ~ "Pitt",
    individualIdSource == "BipSeq"                  ~ "BipSeq",
    TRUE                                            ~ "Not_Used"
...
```{r}
#GVEX matrix is named based on sampleID, whereas CMC is named based on individualID
...
combined_metadata = combined_metadata %>%
  mutate(dataset = case_when(
    ...
    newStudy == "GVEX"                                   ~ "GVEX",
    newStudy == "BipSeq"                                 ~ "BipSeq",
    TRUE                                                 ~ NA_character_
  ))
#Create cohort specific matrices
bipseq_samples = combined_metadata %>% filter(newStudy == "BipSeq") %>% pull(standardID)
...
bipseq_matrix = combined_matrix[, colnames(combined_matrix) %in% bipseq_samples] 
...
#penn_matrix = combined_matrix[, colnames(combined_matrix) %in% penn_samples] #NO CASES
...
```{r}
cell_types = new_marker_list %>% names()

#Define vector of file names to perform MGP on and initialize empty variable before looping
matrix_names = c("nimh_matrix", "gvex_matrix", "mssm_matrix", "pitt_matrix", "bipseq_matrix")
BP_mgp_estimations = setNames(data.frame(matrix(ncol = length(c(colnames(combined_metadata), cell_types)), nrow = 0)), c(colnames(combined_metadata), cell_types))
...
BP_mgp_estimations = rbind(BP_mgp_estimations, estimations_metadata) 
}

write.csv(BP_mgp_estimations, "/external/rprshnas01/kcni/dkiss/cell_prop_psychiatry/data/psychencode_bp_mgp_estimations.csv")
```
